...
test('public health check reports status without opening the workspace', async ({ page }) => {
  await page.goto('/health');
  await expect(page.getByRole('heading', { name: 'StudioFlow Health Check' })).toBeVisible();

  const payload = JSON.parse((await page.getByTestId('health-payload').textContent()) ?? '{}') as {
    status?: string;
    timestamp?: string;
    version?: string;
  };
...
test('layout has no horizontal overflow and primary controls remain touch sized', async ({
  page,
}) => {
...
test('quick capture opens and saves a phone-friendly idea', async ({ page }) => {
  ...
  await page
    .getByLabel(/Idea, line, visual/)
    .fill('A microwave starts a performance review during lunch.');
  await page.getByRole('button', { name: 'Save capture' }).click();
  await expect(
    page.getByText('A microwave starts a performance review during lunch.')
  ).toBeVisible();
});
...
test('manual generation history preserves complete provider provenance', async ({ page }) => {
  ...
  await expect(page.getByRole('status')).toHaveText(
    'Saved generation from Manual Video · cinema-v2.'
  );
...
test('account-free generation simulation completes without provider access and survives reload', async ({
  page,
}) => {
  await page.goto('/episodes/episode-fridge');
  await page.getByRole('tab', { name: 'Prompts & generations' }).click();
...
test('generation results and review decisions persist across reload', async ({ page }) => {
  ...
  const reviewButtons = page
    .getByRole('group', { name: 'Review Example Video cinema-v1' })
    .getByRole('button');
...
test('media details support metadata, production links, and safe deletion confirmation', async ({
  page,
}) => {
