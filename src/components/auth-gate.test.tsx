...
describe("AuthGate owner verification states", () => {
  it("shows genuine owner denial only when the owner RPC returns false", () => {
    ...
    expect(screen.getByRole("heading", { name: "This account is not the StudioFlow owner." })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Try verification again" })).not.toBeInTheDocument();
  });
  ...
  it("shows a retry action for verification errors instead of calling the user a non-owner", async () => {
    ...
    expect(screen.getByRole("heading", { name: "StudioFlow couldn’t verify this sign-in." })).toBeInTheDocument();
