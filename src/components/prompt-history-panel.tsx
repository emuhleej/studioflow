...
import { getEpisodePromptHistory, validatePromptContent, type PromptPurpose } from "../lib/prompt-history";
...
export function PromptHistoryPanel({ episodeId }: { episodeId: string }) {
  ...
  const scenePositions = useMemo(() => new Map(episodeScenes.map((scene, index) => [scene.id, index])), [episodeScenes]);
  const shots = useMemo(
    () => data.shots
  ...
  return (
    <section className="panel panel-pad">
      ...
      <p className="muted mt-2 text-xs leading-5">Every save creates a new immutable version. Older versions stay available for reference.</p>
      <form className="mt-4 grid gap-3" onSubmit={submit}>
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="Purpose">
            <select className="select" value={purpose} onChange={(event) => setPurpose(event.target.value as PromptPurpose)}>
              {purposes.map((item) => <option key={item} value={item}>{titleCase(item)}</option>)}
            </select>
          </Field>
          <Field label="Shot">
            <select className="select" value={shotId} onChange={(event) => setShotId(event.target.value)}>
        ...
        <Field label="Prompt">
          <textarea className="textarea min-h-40" value={content} onChange={(event) => setContent(event.target.value)} placeholder="Record the exact prompt before the next iteration changes it." />
        </Field>
        {error ? <p role="alert" className="text-xs text-[var(--danger)]">{error}</p> : null}
        {message ? <p role="status" className="text-xs text-[var(--mint)]">{message}</p> : null}
        <div className="flex justify-end"><Button variant="primary" type="submit"><CopyPlus size={16} />Save prompt version</Button></div>
      </form>

      {prompts.length ? (
        <div className="mt-5 grid gap-2">
          {prompts.map((prompt) => (
            <article key={prompt.id} className="rounded-xl border border-[var(--line)] bg-black/10 p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="badge">{titleCase(prompt.purpose)} · v{prompt.version}</span>
                  <span className="quiet text-[0.65rem]">{prompt.shotId ? shotsById.get(prompt.shotId)?.title ?? "Unknown shot" : "Episode-wide"}</span>
                </div>
              ...
              <div className="mt-3 flex justify-end">
                ...
                >
                  <Copy size={14} />Use as next draft
                </Button>
              </div>
            </article>
          ))}
        </div>
      ) : <EmptyState icon={<CopyPlus />} title="No prompt versions yet" description="Save the first exact prompt for this episode or one of its shots." />}
    </section>
  );
}
