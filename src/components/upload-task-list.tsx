...
export function UploadTaskList({
  ...
  return (
    <section className="panel panel-pad mb-4" aria-labelledby="upload-tasks-heading">
      ...
      <h2 id="upload-tasks-heading" className="section-title mt-1">
        Upload tasks
      </h2>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {task.status === 'uploading' || task.status === 'cancelling' ? (
                ...
                <div className="muted mt-1 text-xs">
                  {titleCase(task.kind)} · {formatBytes(task.bytes)}
                </div>
                <div className="mt-3 flex items-center gap-3">
                  <div
                    className="progress-track flex-1"
                    aria-label={`${task.filename} upload progress`}
                  >
                    <div
                  ...
                  <span className="quiet w-10 text-right text-xs">
                    {Math.round(task.progress * 100)}%
                  </span>
                </div>
                {task.error ? (
                  <p className="mt-2 text-xs text-[#ffb3ad]" role="alert">
                    {task.error}
                  </p>
                ) : null}
              </div>
              <div className="flex flex-wrap gap-2 sm:justify-end">
                {task.status === 'uploading' ? (
                  <Button onClick={() => onPause(task.id)}>
                    <Pause size={15} />
                    Pause
                  </Button>
                ) : null}
                {task.status === 'paused' ? (
                  <Button variant="primary" onClick={() => onResume(task.id)}>
                    <Play size={15} />
                    Resume
                  </Button>
                ) : null}
                ...
                {['queued', 'uploading', 'paused', 'failed'].includes(task.status) ? (
                ...
                {['completed', 'cancelled'].includes(task.status) ? (
                  <Button onClick={() => onDismiss(task.id)}>Dismiss</Button>
                ) : null}
              </div>
            </div>
          </article>
