...
import {
  createMediaAccess,
  downloadMediaAsset,
  getMediaRefreshDelay,
  type MediaAccessHandle,
...
export function MediaPreview({ asset, controls = false }: { asset: Asset; controls?: boolean }) {
  ...
  if (asset.source === 'demo' || asset.storageKey.startsWith('fake://'))
    return <MediaPlaceholder asset={asset} />;
  if (error) {
    return (
  ...
  if (!access)
    return (
      <div className="media-placeholder" aria-label={`Loading ${asset.filename}`}>
  ...
  if (asset.kind === 'image')
    return (
      <div className="media-placeholder">
        <img src={access.url} alt={asset.filename} onError={recoverPlayback} />
      </div>
    );
  if (asset.kind === 'video')
    return (
  ...
  return (
    <div className="media-placeholder">
      <audio src={access.url} controls aria-label={asset.filename} onError={recoverPlayback} />
    </div>
  );
}
...
export function MediaDownloadButton({ asset }: { asset: Asset }) {
  ...
  return (
    <div className="grid gap-1">
      ...
      {error ? (
        <span className="text-xs text-[#ffb3ad]" role="alert">
          {error}
        </span>
      ) : null}
    </div>
