...
function Navigation({ mobile = false }: { mobile?: boolean }) {
  return (
    <nav
      className={mobile ? 'mobile-nav' : 'grid gap-1'}
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="mt-auto grid gap-3">
          <Button variant="primary" aria-label="Quick capture" onClick={() => setCaptureOpen(true)}>
            <Plus size={17} />
            <span className="sidebar-copy">Quick capture</span>
          </Button>
          <div className="sidebar-copy rounded-xl border border-[var(--line)] p-3">
            ...
            <div className="muted mt-2 truncate text-xs">
              {activeProject?.title ?? 'No active project'}
            </div>
            <div className="quiet mt-1 text-[0.65rem]">
              {isDemo ? 'Fictional demo workspace' : 'Private cloud workspace'}
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="min-w-0">
        <header className="mobile-topbar">
          <div className="flex items-center gap-2">
            ...
            <strong className="text-sm">StudioFlow</strong>
          </div>
          <Button
          ...
          <div className="page-wrap">
            <header className="topbar">
              <div className="flex min-w-0 items-center gap-2 text-xs">
                <span className="muted">StudioFlow</span>
              ...
              <div className="flex items-center gap-2">
                <span className="badge">
                  <span
                    ...
                    style={{ color: isDemo ? 'var(--amber)' : 'var(--mint)' }}
                  />
                  {isDemo ? 'Demo data' : 'Owner protected'}
                </span>
      ...
      <Modal
        ...
        title="Quick capture"
        description="Save the thought now; shape it into an episode later."
      >
        <form className="grid gap-4" onSubmit={submitCapture}>
          <Field label="Idea, line, visual, or production note">
            <textarea
              ...
              placeholder="A smart vacuum files a workplace complaint…"
            />
          </Field>
