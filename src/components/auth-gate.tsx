import { LockKeyhole, ShieldAlert, ShieldX, Sparkles } from 'lucide-react';
      ...
      <main className="grid min-h-screen place-items-center p-5">
        <section className="panel w-full max-w-md p-7 text-center" role="alert">
          ...
          <h1 className="mt-4 text-2xl font-semibold text-white">
            StudioFlow couldn’t verify this sign-in.
          </h1>
          <p className="muted mt-3 text-sm leading-6">
            Your account may still be the owner. The verification request failed, so StudioFlow did
            not make an access decision.
          </p>
          <div className="mt-5 grid gap-3">
            <Button variant="primary" className="w-full" onClick={retryOwnerVerification}>
  ...
  if (authLoading || dataLoading || (user && ownerAuthorized === null)) {
    return (
      <div className="grid min-h-screen place-items-center">
        <div className="grid justify-items-center gap-3 muted">
          <LoadingSpinner label="Opening your studio" />
          <span className="text-sm">Opening your studio…</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <main className="grid min-h-screen place-items-center p-5">
        <section className="panel w-full max-w-md p-7 text-center" role="alert">
          ...
          <h1 className="mt-4 text-2xl font-semibold text-white">
            This account is not the StudioFlow owner.
          </h1>
          <p className="muted mt-3 text-sm leading-6">
            The page is public, but the workspace only opens for the single UUID stored in the owner
            allowlist.
          </p>
    ...
    <main className="grid min-h-screen place-items-center p-5">
      <section className="panel w-full max-w-md overflow-hidden">
        <div className="border-b border-[var(--line)] p-7">
          ...
          <p className="muted mt-3 text-sm leading-6">
            Your stories, production memory, media, and costs stay behind an owner-only GitHub
            sign-in.
          </p>
        </div>
        <div className="grid gap-4 p-7">
          <div className="flex items-start gap-3 rounded-xl border border-[var(--line)] bg-black/10 p-3">
            ...
            <p className="muted text-xs leading-5">
              Authentication is handled by Supabase. Database policies and private media URLs
              enforce access even if someone discovers this page.
            </p>
          </div>
