...
describe('generation history', () => {
  ...
  it('requires provider and model names', () => {
    expect(validateGenerationInput(demoWorkspace, { ...validInput, provider: '  ' })).toBe(
      'Enter the generation provider.'
    );
    expect(validateGenerationInput(demoWorkspace, { ...validInput, model: '' })).toBe(
      'Enter the model name.'
    );
  });

  it('rejects invalid cost and duration values', () => {
    expect(validateGenerationInput(demoWorkspace, { ...validInput, costCents: -1 })).toBe(
      'Cost must be zero or a positive number of cents.'
    );
    expect(validateGenerationInput(demoWorkspace, { ...validInput, costCents: 1.5 })).toBe(
      'Cost must be zero or a positive number of cents.'
    );
    expect(validateGenerationInput(demoWorkspace, { ...validInput, durationSeconds: 0 })).toBe(
      'Duration must be between 0 and 14,400 seconds.'
    );
  });

  it('keeps shot and prompt references inside the episode', () => {
    expect(
      validateGenerationInput(demoWorkspace, { ...validInput, episodeId: 'episode-thermostat' })
    ).toBe('Choose a shot from this episode.');
    expect(validateGenerationInput(demoWorkspace, { ...validInput, shotId: 'shot-2' })).toBe(
      'The generation shot must match the selected prompt version.'
    );
  });
  ...
  it("sorts one episode's records newest first", () => {
    ...
    const other = {
      ...newer,
      id: 'generation-other',
      episodeId: 'episode-laundry',
      createdAt: '2026-03-01T00:00:00.000Z',
    };
    expect(
      getEpisodeGenerationHistory([older, newer, other], 'episode-fridge').map((item) => item.id)
    ).toEqual(['generation-new', older.id]);
  });

  it('builds a readable immutable prompt reference label', () => {
    expect(getPromptVersionLabel(demoWorkspace.prompts[1], 'Immediate argument')).toBe(
      'Video v2 · Immediate argument'
    );
  });
  ...
  it('combines legacy result IDs and explicit generation links without duplicates', () => {
    ...
    expect(
      getGenerationResultAssets(workspace, 'generation-hook-v3').map((asset) => asset.id)
    ).toEqual(['asset-shot-one']);
  });
  ...
  it('offers active media from the generation project and rejects invalid links', () => {
    ...
    workspace.assets.push({
      ...workspace.assets[0],
      id: 'other-project-result',
      projectId: 'other-project',
    });

    const eligibleIds = getEligibleGenerationAssets(workspace, 'generation-hook-v3').map(
      (asset) => asset.id
    );
    ...
    expect(validateGenerationAssetLink(workspace, 'generation-hook-v3', 'trashed-result')).toBe(
      'Restore this media before linking it as a result.'
    );
    expect(
      validateGenerationAssetLink(workspace, 'generation-hook-v3', 'other-project-result')
    ).toBe('Choose media from the same project as this generation.');
  });
  ...
  it('accepts only decisions for an existing generation', () => {
    ...
    expect(validateGenerationOutcome(demoWorkspace, 'missing-generation', 'rejected')).toBe(
