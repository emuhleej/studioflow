...
export type UploadTaskStatus = "queued" | "uploading" | "paused" | "failed" | "cancelling" | "cancelled" | "completed";
...
export function transitionUploadTask(task: UploadTask, event: UploadTaskEvent): UploadTask {
  switch (event.type) {
    ...
    case "progress":
      ...
      return { ...task, progress: Math.max(task.progress, Math.min(1, Math.max(0, event.progress))) };
    case "pause":
    ...
    case "cancelled":
      return task.status === "completed" ? task : { ...task, status: "cancelled", error: undefined };
    case "complete":
      return { ...task, status: "completed", progress: 1, error: undefined, assetId: event.assetId };
  }
}
