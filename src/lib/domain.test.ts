...
describe('StudioFlow domain calculations', () => {
  ...
  it('blocks oversized files and uploads that cross the 9 GB guardrail', () => {
    ...
    expect(
      canAcceptAsset(nearCap, { bytes: 11, kind: 'video', mimeType: 'video/mp4' }).allowed
    ).toBe(false);
    expect(
      canAcceptAsset(nearCap, { bytes: 10, kind: 'video', mimeType: 'video/mp4' }).allowed
    ).toBe(true);
  });

  it('accepts only non-empty files whose MIME type matches their media kind', () => {
    expect(
      canAcceptAsset(demoWorkspace, { bytes: 0, kind: 'image', mimeType: 'image/png' })
    ).toEqual({
      allowed: false,
      reason: 'Choose a file that is not empty.',
    });
    expect(
      canAcceptAsset(demoWorkspace, { bytes: 100, kind: 'image', mimeType: 'image/svg+xml' })
    ).toEqual({
      allowed: false,
      reason: 'Unsupported image format: image/svg+xml.',
    });
    expect(
      canAcceptAsset(demoWorkspace, { bytes: 100, kind: 'audio', mimeType: 'video/mp4' }).allowed
    ).toBe(false);
  ...
  it('removes an asset and every embedded or explicit reference', () => {
    ...
    expect(
      cleaned.generations.some((generation) => generation.assetIds.includes('asset-shot-one'))
    ).toBe(false);
  });
