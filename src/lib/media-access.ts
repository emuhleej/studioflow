...
export async function createMediaAccess(
  ...
  if (isDemo) {
    ...
    if (!blob)
      throw new Error(
        'This browser no longer has the media file. Upload it again to restore access.'
      );
    const url = URL.createObjectURL(blob);
    return { url, release: () => URL.revokeObjectURL(url) };
  }

  const access = await getRemoteAssetAccess(
    asset.id,
    purpose === 'download' ? 'attachment' : 'inline'
  );
  return {
    url: access.url,
...
export function getMediaRefreshDelay(
