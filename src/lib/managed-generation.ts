...
export function getGenerationProjectId(workspace: WorkspaceData, generation: GenerationRecord): string | undefined {
...
export function prepareManagedGeneration(
  ...
  const prompt = workspace.prompts.find((item) => item.id === input.promptVersionId && item.episodeId === input.episodeId);
  if (!prompt) throw new Error("Choose an immutable prompt version from this episode.");
  if (prompt.shotId && prompt.shotId !== input.shotId) throw new Error("The generation shot must match the prompt version.");
  if (input.shotId) {
    ...
    if (!scene || scene.episodeId !== input.episodeId) throw new Error("Choose a shot from this episode.");
  }
  if (input.settings.outputCount !== 1) throw new Error("StudioFlow permits one generated output per request.");
  if (input.mediaKind === "video" && !input.references.some((reference) => reference.role === "start_image")) {
    throw new Error("Video simulation requires one approved starting image.");
  }

  const capabilities = provider.capabilities();
  const model = capabilities.models.find((candidate) => candidate.id === input.model && candidate.mediaKind === input.mediaKind);
  if (!model) throw new Error("Choose a model that supports this media type.");
  if (!model.aspectRatios.includes(input.settings.aspectRatio)) throw new Error("The model does not support this aspect ratio.");
  if (input.mediaKind === "video" && !model.durations.includes(input.settings.durationSeconds ?? 0)) {
  ...
  for (const reference of input.references) {
    ...
    if (seenReferences.has(dedupeKey)) throw new Error("The same reference role cannot be added twice.");
    seenReferences.add(dedupeKey);
    const asset = workspace.assets.find((candidate) => candidate.id === reference.assetId);
    if (!asset || asset.projectId !== series.projectId || asset.kind !== "image" || asset.deletedAt) {
      throw new Error("References must be active images from this project.");
    }
    if (asset.bytes > budget.referenceImageLimitBytes) throw new Error("Reference images must be 16 MB or smaller.");
...
function appendStatusEvent(
  ...
  return [...workspace.generationEvents, {
    ...baseRecord(workspace.ownerId, runtime),
    generationId: generation.id,
...
function updateGeneration(
  ...
  return {
    ...workspace,
    generations: workspace.generations.map((generation) => generation.id === generationId ? updater(generation) : generation),
  };
}
...
export function claimManagedGeneration(
  ...
  const active = workspace.generations.find((item) => item.id !== generation.id && item.executionMode === "managed"
  ...
  const requestLimit = generation.mediaKind === "video" ? budget.maxVideoRequestMicros : budget.maxImageRequestMicros;
  if (generation.estimatedCostMicros > requestLimit) throw new Error("Generation exceeds the per-request spending limit.");

  const nowValue = new Date(runtime.now());
  const costExposure = (candidate: GenerationRecord) => candidate.calculatedCostMicros
    ?? (ACTIVE_GENERATION_STATUSES.includes(candidate.operationalStatus) ? candidate.reservedMaxCostMicros : 0);
  ...
  const monthlyExposure = workspace.generations
    ...
    .filter((item) => {
      ...
      return created.getUTCFullYear() === nowValue.getUTCFullYear() && created.getUTCMonth() === nowValue.getUTCMonth();
    })
    .reduce((sum, item) => sum + costExposure(item), 0);
  if (dailyExposure + generation.estimatedCostMicros > budget.dailyLimitMicros) throw new Error("Generation exceeds the daily spending limit.");
  if (monthlyExposure + generation.estimatedCostMicros > budget.monthlyLimitMicros) throw new Error("Generation exceeds the monthly spending limit.");

  const usedBytes = workspace.assets.reduce((sum, asset) => sum + asset.bytes, 0);
  const reservedBytes = workspace.generations
    .filter((item) => item.id !== generation.id && ACTIVE_GENERATION_STATUSES.includes(item.operationalStatus))
    .reduce((sum, item) => sum + item.reservedOutputBytes, 0);
  if (generation.estimatedOutputBytes > budget.generatedOutputLimitBytes
  ...
  return {
    ...updated,
    generationEvents: appendStatusEvent(workspace, generation, "draft", "submitting", "Generation claim reserved safely.", runtime),
  };
}
...
export function markProviderSubmissionStarted(
  ...
  const generation = workspace.generations.find((item) => item.id === generationId);
  if (!generation || generation.operationalStatus !== "submitting" || generation.submissionClaimId !== claimId) {
  ...
  return {
    ...updated,
    generationEvents: [...workspace.generationEvents, {
      ...baseRecord(workspace.ownerId, runtime),
      generationId,
...
export function transitionManagedGeneration(
  ...
  if (!isGenerationTransitionAllowed(generation.operationalStatus, toStatus)) {
    throw new Error(`Invalid generation transition from ${generation.operationalStatus} to ${toStatus}.`);
  }
  const timestamp = runtime.now();
  const next: GenerationRecord = {
    ...
    updatedAt: timestamp,
    reservedMaxCostMicros: ["failed", "cancelled", "completed"].includes(toStatus) ? 0 : (patch.reservedMaxCostMicros ?? generation.reservedMaxCostMicros),
    reservedOutputBytes: ["failed", "cancelled", "completed"].includes(toStatus) ? 0 : (patch.reservedOutputBytes ?? generation.reservedOutputBytes),
    completedAt: ["failed", "cancelled", "completed"].includes(toStatus) ? timestamp : generation.completedAt,
  };
  const updated = updateGeneration(workspace, generationId, () => next);
  return {
    ...updated,
    generationEvents: appendStatusEvent(workspace, generation, generation.operationalStatus, toStatus, message, runtime),
  };
}
...
export function attachProviderJob(
  ...
  return transitionManagedGeneration(workspace, generationId, "queued", {
    providerJobId,
    submissionClaimId: undefined,
    submissionClaimExpiresAt: undefined,
    nextPollAt: runtime.now(),
  }, "Provider accepted the generation job.", runtime);
}
...
export function completeManagedGeneration(
  ...
  if (generation.operationalStatus !== "saving") throw new Error("Generation must be saving before it can complete.");
  ...
  if (!projectId || !generation.mediaKind) throw new Error("Generation project context is missing.");

  const existingAsset = workspace.assets.find((asset) => asset.storageKey === `fake://${generation.id}`);
  const asset: Asset = existingAsset ?? {
    ...
    durationSeconds: generation.mediaKind === "video" ? generation.requestSettings.durationSeconds ?? null : null,
    width: null,
    height: null,
  ...
  const existingLink = workspace.assetLinks.find((link) => link.targetType === "generation"
    && link.targetId === generation.id && link.assetId === asset.id);
  ...
  const existingCost = workspace.costEntries.find((entry) => entry.sourceGenerationId === generation.id);
  ...
  const withRelationships: WorkspaceData = {
    ...
    generations: workspace.generations.map((item) => item.id === generation.id ? {
      ...item,
      assetIds: item.assetIds.includes(asset.id) ? item.assetIds : [...item.assetIds, asset.id],
      calculatedCostMicros,
      costCents: cost.amountCents,
      ingestAttempts: item.ingestAttempts + 1,
    } : item),
  };
  return transitionManagedGeneration(withRelationships, generation.id, "completed", {
    calculatedCostMicros,
    costCents: cost.amountCents,
    completedAt: runtime.now(),
  }, "Generated output was saved and linked exactly once.", runtime);
}
...
export function recoverInterruptedSubmission(
  ...
  if (generation.submissionClaimExpiresAt && generation.submissionClaimExpiresAt > runtime.now()) return workspace;
  if (!generation.providerSubmissionStartedAt) {
    return transitionManagedGeneration(workspace, generationId, "draft", {
      submissionClaimId: undefined,
      submissionClaimExpiresAt: undefined,
      reservedMaxCostMicros: 0,
      reservedOutputBytes: 0,
    }, "Expired pre-submission claim was released safely.", runtime);
  }
  return transitionManagedGeneration(workspace, generationId, "submission_unknown", {
    submissionClaimId: undefined,
    submissionClaimExpiresAt: undefined,
    failureCode: "submission_response_lost",
    failureMessage: "Provider submission may have occurred; owner review is required.",
  }, "Submission outcome is unknown; reservations remain held.", runtime);
}
...
export function resolveSubmissionUnknown(
  ...
  if (!Number.isSafeInteger(confirmedCostMicros) || confirmedCostMicros < 0 || confirmedCostMicros > generation.reservedMaxCostMicros) {
    throw new Error("Confirmed cost must fit inside the reserved maximum.");
  }
  const charged = outcome === "confirmed_charge";
  let next = workspace;
  if (charged && generation.mediaKind && !workspace.costEntries.some((entry) => entry.sourceGenerationId === generation.id)) {
    next = {
      ...workspace,
      costEntries: [...workspace.costEntries, {
        ...baseRecord(workspace.ownerId, runtime),
        episodeId: generation.episodeId,
        sourceGenerationId: generation.id,
        category: generation.mediaKind,
  ...
  return transitionManagedGeneration(next, generation.id, charged ? "failed" : "cancelled", {
    ...
    failureMessage: charged ? "Provider charge was confirmed without a recoverable result." : undefined,
  }, charged ? "Unknown submission was closed with a confirmed charge." : "Unknown submission was confirmed as not charged.", runtime);
}
