...
import { RUNWAY_API_VERSION, RunwayGenerationProvider } from "../../supabase/functions/_shared/runway";
...
describe("mocked Runway adapter", () => {
  it("maps a normalized image request to the versioned API without returning signed references", async () => {
    ...
    expect(job).toEqual({ providerJobId: "task-1", status: "queued", createdAt: "2026-09-02T12:00:00.000Z" });
    ...
    expect(JSON.parse(String(init.body))).toEqual({
      ...
      ratio: "720:1280",
      referenceImages: [{ uri: "https://private-media.example.com/signed-reference?token=secret", tag: "Reference1" }],
    });
    expect(JSON.stringify(job)).not.toContain("signed-reference");
  });

  it("uses the explicitly assigned starting image for video", async () => {
    const fetcher = vi.fn(async () => new Response(JSON.stringify({ id: "task-video", status: "PENDING" }), { status: 200 })) as unknown as typeof fetch;
    const provider = new RunwayGenerationProvider("server-secret", {
      allowedHost: "private-media.example.com",
