...
export function getNextPromptVersion(
  ...
  const versions = prompts
    .filter((prompt) => prompt.episodeId === episodeId && prompt.shotId === shotId && prompt.purpose === purpose)
    .map((prompt) => prompt.version);

  return versions.length ? Math.max(...versions) + 1 : 1;
}

export function getEpisodePromptHistory(prompts: PromptVersion[], episodeId: string): PromptVersion[] {
  return prompts
