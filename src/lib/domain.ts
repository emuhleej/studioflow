...
export function getSitcomShotDurations(targetDurationSeconds: number): number[] {
  ...
  const durations = SITCOM_SHOT_WEIGHTS.map((weight) =>
    Math.max(4, Math.round((weight / totalWeight) * target))
  );
...
export function getEpisodeAssets(data: WorkspaceData, episodeId: string): Asset[] {
  ...
  return data.assets.filter(
    (asset) => asset.episodeId === episodeId || linkedAssetIds.has(asset.id)
  );
}
...
export function canAcceptAsset(
  ...
  if (candidate.bytes > MAX_ASSET_BYTES)
    return { allowed: false, reason: 'Files must be 2 GB or smaller.' };
  if (!ACCEPTED_MEDIA_MIME_TYPES[candidate.kind].includes(candidate.mimeType.toLowerCase())) {
    return {
      allowed: false,
      reason: `Unsupported ${candidate.kind} format: ${candidate.mimeType || 'unknown'}.`,
    };
  }
...
export function getAssetLinkOptions(data: WorkspaceData, asset: Asset): AssetLinkOption[] {
  ...
  return [
    ...data.projects
      .filter((item) => item.id === asset.projectId)
      .map((item) => ({ targetType: 'project' as const, targetId: item.id, label: item.title })),
    ...series.map((item) => ({
    ...
    ...episodes.map((item) => ({
      ...
      targetId: item.id,
      label: `E${String(item.number).padStart(2, '0')} · ${item.title}`,
    })),
    ...scenes.map((item) => ({
    ...
    ...data.shots
      ...
      .map((item) => ({ targetType: 'shot' as const, targetId: item.id, label: item.title })),
    ...data.entities
      .filter((item) => item.projectId === asset.projectId)
      .map((item) => ({ targetType: 'entity' as const, targetId: item.id, label: item.name })),
    ...data.generations
      .filter((item) => episodeIds.has(item.episodeId))
      .map((item) => ({
