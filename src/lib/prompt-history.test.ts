...
describe('prompt history', () => {
  it('increments versions within an episode, purpose, and shot chain', () => {
    expect(getNextPromptVersion(demoWorkspace.prompts, 'episode-fridge', 'video', 'shot-1')).toBe(
      3
    );
    expect(getNextPromptVersion(demoWorkspace.prompts, 'episode-fridge', 'image', 'shot-1')).toBe(
      1
    );
