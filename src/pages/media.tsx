...
export function MediaPage() {
  ...
  const confirmPermanentDelete = async () => {
    ...
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : 'The media could not be permanently deleted.'
      );
    } finally {
      setDeleteBusy(false);
    }
  ...
  return (
    <div>
        ...
        actions={
          <>
            <input
              ...
              className="hidden"
              type="file"
              accept="image/*,audio/*,video/*,.mov,.m4a"
              onChange={chooseFile}
            />
            <Button variant="primary" onClick={() => inputRef.current?.click()} disabled={!project}>
              <Upload size={16} />
              Upload media
            </Button>
          </>
      ...
      {error ? (
        <div
          className="mb-4 rounded-xl border border-[rgb(240_125_118_/_0.35)] bg-[rgb(240_125_118_/_0.08)] p-3 text-sm text-[#ffb3ad]"
          role="alert"
        >
          {error}
        </div>
      ...
      <UploadTaskList
        ...
        onRetry={retryUpload}
        onCancel={(taskId) => void cancelUpload(taskId)}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0 flex-1">
            <div className="mb-2 flex justify-between text-xs">
              <span className="muted">Storage safety meter</span>
              <span>{formatBytes(storage)} / 9 GB</span>
            </div>
            <div className="progress-track">
              <div
                className="progress-fill"
                style={{ width: `${Math.min(100, (storage / B2_UPLOAD_BLOCK_BYTES) * 100)}%` }}
              />
            </div>
          </div>
          <span className="badge">
            {formatBytes(Math.max(0, B2_UPLOAD_BLOCK_BYTES - storage))} available
          </span>
        </div>
      </section>
      ...
      {assets.length ? (
          ...
          {assets.map((asset) => (
            <article key={asset.id} className="panel overflow-hidden p-3">
              <button className="block w-full text-left" onClick={() => setSelectedId(asset.id)}>
                ...
                <div className="mt-3">
                  <div className="truncate text-sm font-semibold">{asset.filename}</div>
              ...
              <div className="mt-3 flex gap-2">
                {filter === 'active' ? (
                  <>
                    <Button className="flex-1" onClick={() => setAssetReview(asset.id, 'selected')}>
                      <Check size={15} />
                      Select
                    </Button>
                    <Button
                  ...
                  <>
                    <Button className="flex-1" onClick={() => restoreAsset(asset.id)}>
                      <RotateCcw size={15} />
                      Restore
                    </Button>
                    <Button
              ...
              <div className="mt-2 flex justify-between">
                <span className="badge">{titleCase(asset.reviewStatus)}</span>
                <span className="quiet text-[0.65rem]">{asset.source}</span>
              </div>
            </article>
          ))}
        </div>
        ...
        <section className="panel">
          <EmptyState
            icon={filter === 'trash' ? <Trash2 size={28} /> : <ImagePlus size={28} />}
            title={filter === 'trash' ? 'Trash is empty' : 'No media found'}
            description={
              filter === 'trash'
                ? 'Removed media stays recoverable here until you permanently delete it.'
                : 'Upload an image, audio file, or video to begin building the asset library.'
            }
            action={
              filter === 'active' ? (
      ...
      <Modal
        ...
        title={selected?.filename ?? 'Media detail'}
        description={
          selected ? `${titleCase(selected.kind)} · ${formatBytes(selected.bytes)}` : undefined
        }
      >
      ...
      >
        <p className="muted text-sm leading-6">
          {deleteCandidate?.filename} will no longer be recoverable from StudioFlow.
        </p>
      </Modal>
    </div>
  );
...
function AssetDetail({ asset }: { asset: Asset }) {
  ...
  const options = allOptions.filter(
    (option) => !linkedKeys.has(`${option.targetType}|${option.targetId}`)
  );
  const [targetKey, setTargetKey] = useState(
    options[0] ? `${options[0].targetType}|${options[0].targetId}` : ''
  );
  const effectiveTargetKey = options.some(
    (option) => `${option.targetType}|${option.targetId}` === targetKey
  )
    ? targetKey
    : options[0]
      ? `${options[0].targetType}|${options[0].targetId}`
      : '';
  const labels = new Map(
    allOptions.map((option) => [`${option.targetType}|${option.targetId}`, option.label])
  );
  ...
  const linkTarget = () => {
    ...
    try {
      ...
      setMessage(
        reason instanceof Error ? reason.message : 'The production link could not be added.'
      );
    }
  };
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="flex flex-wrap items-start gap-2">
        <MediaDownloadButton asset={asset} />
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="Review">
            <select
              ...
              onChange={(event) =>
                setAssetReview(asset.id, event.target.value as Asset['reviewStatus'])
              }
            >
          ...
          <Field label="Source">
            <input className="input" value={titleCase(asset.source)} disabled />
          </Field>
        </div>
        <Field label="Notes">
          <textarea
      ...
      <section className="rounded-xl border border-[var(--line)] bg-black/10 p-3">
        <div className="flex items-center gap-2">
          <Link2 size={16} color="var(--violet)" />
          <h3 className="text-sm font-semibold">Production links</h3>
        </div>
        {links.length ? (
            ...
            {links.map((link) => (
              <div key={link.id} className="list-row">
                <div className="min-w-0">
                  <div className="text-xs font-semibold">{titleCase(link.targetType)}</div>
                  <div className="muted mt-1 truncate text-xs">
                    {labels.get(`${link.targetType}|${link.targetId}`) ?? link.targetId}
                  </div>
                </div>
                <Button
                  className="icon-button"
                  aria-label={`Remove ${link.targetType} link`}
                  onClick={() =>
                    void removeAssetLink(link.id).catch((reason: unknown) =>
                      setMessage(
                        reason instanceof Error ? reason.message : 'The link could not be removed.'
                      )
                    )
                  }
                >
          ...
          <p className="muted mt-2 text-xs">This media has no explicit production links yet.</p>
        )}
          ...
          >
            {options.length ? (
              options.map((option) => (
                ...
                >
                  {titleCase(option.targetType)} · {option.label}
                </option>
              ))
            ) : (
              <option value="">All available records are linked</option>
            )}
          </select>
          <Button onClick={linkTarget} disabled={!options.length}>
            <Link2 size={15} />
            Add link
          </Button>
        </div>
      ...
      {message ? (
        <p className="text-xs text-[var(--mint)]" role="status">
          {message}
        </p>
      ) : null}
    </div>
