...
describe('owner authorization', () => {
  it('stabilizes the session and retries one transient 401', async () => {
    ...
    await expect(verifyOwnerAuthorization(verify, stabilizeSession)).resolves.toEqual({
      status: 'allowed',
    });
  ...
  it('does not let a stale 401 overwrite a newer 200 result', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ sessionKey }) =>
        useOwnerAuthorization({ enabled: true, sessionKey, verify, stabilizeSession }),
  ...
  it('clears an allowed result when the authenticated session is removed', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ enabled, sessionKey }) =>
        useOwnerAuthorization({ enabled, sessionKey, verify, stabilizeSession }),
  ...
  it('keeps a newly authorized session behind the gate until its workspace load settles', () => {
    ...
    expect(
      isOwnerWorkspaceLoading(false, { status: 'allowed' }, 'new-session', 'old-session')
    ).toBe(true);
    expect(
      isOwnerWorkspaceLoading(false, { status: 'allowed' }, 'new-session', 'new-session')
    ).toBe(false);
  });
});
