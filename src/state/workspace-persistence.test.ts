...
describe("workspace persistence", () => {
  ...
  it("normalizes legacy exports and preserves unknown record fields", () => {
    ...
    expect(restored.generations[0]).toMatchObject({ executionMode: "manual", operationalStatus: "recorded" });
  ...
  it("restores generation targets before their polymorphic asset links", () => {
    ...
    expect(workspaceCollectionKeys.indexOf("generations")).toBeLessThan(workspaceCollectionKeys.indexOf("generationInputs"));
    expect(workspaceCollectionKeys.indexOf("generationInputs")).toBeLessThan(workspaceCollectionKeys.indexOf("assetLinks"));
  });
});
