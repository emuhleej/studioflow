...
import type { BaseRecord, GenerationBudgetSettings, GenerationRecord, WorkspaceData } from "../types";
...
function normalizeGeneration(record: GenerationRecord): GenerationRecord {
  ...
  return {
    ...
    operationalStatus: record.operationalStatus ?? (executionMode === "managed" ? "draft" : "recorded"),
...
export function normalizeWorkspace(workspace: WorkspaceData): WorkspaceData {
  ...
  return {
    ...
    generations: Array.isArray(workspace.generations) ? workspace.generations.map(normalizeGeneration) : [],
    ...
    generationBudgetSettings: generationBudgetSettings.length
      ? generationBudgetSettings.map((settings) => ({ ...settings, generationEnabled: settings.generationEnabled === true }))
...
const workspaceExportSchema = z.object({
  ...
  generationInputs: z.array(z.object({ id: z.string() }).passthrough()).optional().default([]),
  generationEvents: z.array(z.object({ id: z.string() }).passthrough()).optional().default([]),
  generationBudgetSettings: z.array(z.object({ id: z.string() }).passthrough()).optional().default([]),
