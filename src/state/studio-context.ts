...
export interface StudioContextValue {
  ...
  createSeries: (
    input: Pick<
      Series,
      'projectId' | 'title' | 'premise' | 'format' | 'orientation' | 'targetDurationSeconds'
    >
  ) => Series;
  createEpisode: (seriesId: string, title: string, idea: string) => Episode;
  updateEpisode: (
    episodeId: string,
    patch: Partial<Pick<Episode, 'title' | 'idea' | 'status' | 'targetDurationSeconds' | 'tags'>>
  ) => void;
  createEntity: (
    input: Pick<
      ProductionEntity,
      'projectId' | 'kind' | 'name' | 'summary' | 'promptFragment' | 'accent'
    > & {
  ...
  updateScene: (
    sceneId: string,
    patch: Partial<Pick<Scene, 'title' | 'summary' | 'beat' | 'locationId' | 'position'>>
  ) => void;
  ...
  addTimeEntry: (
    episodeId: string,
    minutes: number,
    category: 'script' | 'storyboard' | 'generation' | 'editing' | 'publishing' | 'idea' | 'other',
    note: string
  ) => void;
  addCostEntry: (
    episodeId: string,
