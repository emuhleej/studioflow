...
try {
  ...
  await desktop.screenshot({ path: resolve(outputDirectory, "studioflow-creator-hq.png"), fullPage: true });
  ...
  await phone.screenshot({ path: resolve(outputDirectory, "studioflow-phone-capture.png"), fullPage: false });
} finally {
  await browser.close();
}
