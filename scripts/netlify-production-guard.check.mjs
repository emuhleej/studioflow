...
import { isProductionReleaseApproved, runProductionGuard } from './netlify-production-guard.mjs';
...
test('similar values cannot accidentally unlock production', () => {
  assert.equal(
    isProductionReleaseApproved({ STUDIOFLOW_PRODUCTION_RELEASE_COMMIT: 'approved' }),
    false
  );
  assert.equal(
...
test('Netlify keeps previews on the normal build and production behind both guards', () => {
  ...
  assert.match(
    production,
    /command\s*=\s*"node scripts\/netlify-production-guard\.mjs && npm run build"/
  );
