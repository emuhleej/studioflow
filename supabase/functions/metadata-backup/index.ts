...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const ownerIds = authenticated
      ...
      : ((await admin.from('app_owners').select('user_id')).data ?? []).map(
          (owner) => owner.user_id as string
        );
    ...
    for (const ownerId of ownerIds) {
      ...
      const { data: priorBackups } = await admin
        .from('backup_runs')
        .select('bytes')
        .eq('owner_id', ownerId)
        .eq('status', 'completed');
      const usedBytes = [...(assets ?? []), ...(priorBackups ?? [])].reduce(
        (sum, record) => sum + Number(record.bytes),
        0
      );
      if (usedBytes + encrypted.byteLength > UPLOAD_BLOCK_BYTES)
        throw new Error('The 9 GB storage safety cap prevents this backup.');
      ...
      await b2Client().send(
        new PutObjectCommand({
          ...
          Metadata: {
            ...
            envelope: '12-byte-nonce-plus-ciphertext',
            schema: '2',
          },
        })
      );
