...
import { loadGenerationRow, nextGenerationPoll, runwayRequestContext } from "../_shared/generation-service.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: marked, error: markerError } = await admin.rpc("mark_generation_submission_started", {
      target_generation_id: generation.id,
      requested_claim_id: claimId,
      target_owner_id: user.id,
    });
    if (markerError || !marked) throw markerError ?? new Error("Generation submission marker was not recorded.");
