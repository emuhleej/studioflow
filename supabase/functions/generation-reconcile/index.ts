...
async function invokeIngest(generationId: string): Promise<void> {
  ...
  if (!baseUrl || !secret)
    throw new Error('Generation ingest service configuration is incomplete.');
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: recovered, error: recoveryError } = await admin.rpc(
      'recover_stale_generation_claims'
    );
    ...
    const { data: due, error: dueError } = await admin
      ...
      .select(
        'id, owner_id, operational_status, provider_job_id, poll_attempts, ingest_attempts, estimated_cost_micros'
      )
    ...
    for (const generation of due ?? []) {
      ...
      if (state.status === 'queued' || state.status === 'running') {
        const nextStatus =
          state.status === 'running' && generation.operational_status === 'queued'
            ? 'running'
      ...
      if (state.status === 'cancelled') {
        const { error } = await admin.rpc('finalize_generation_without_output', {
          ...
          target_failure_message:
            'Provider cancellation was confirmed; charge status requires owner review.',
        });
        if (error) throw error;
        attentionRequired += 1;
        continue;
      ...
      const { error } = await admin.rpc('finalize_generation_without_output', {
        ...
        target_failure_message: safeProviderText(
          state.failureMessage,
          'The provider generation failed without a saved output.'
        ),
      });
      if (error) throw error;
    }
