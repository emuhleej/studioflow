...
export async function loadGenerationRow(
  ...
  if (row.provider !== "runway" || !row.prompt_version_id || !row.client_request_id || !row.media_kind) {
...
async function loadSignedReferences(
...
): Promise<{ resolver: RunwayReferenceResolver; references: NormalizedGenerationRequest["references"] }> {
  ...
  for (const input of inputs ?? []) {
    const { data: asset, error: assetError } = await admin
      ...
      .eq("owner_id", generation.owner_id)
      .single();
    if (assetError || !asset || asset.kind !== "image" || asset.deleted_at || Number(asset.bytes) > 16_000_000) {
...
export async function runwayRequestContext(
  ...
  const provider = new RunwayGenerationProvider(requiredEnvironment("RUNWAYML_API_SECRET"), resolver);
  return {
    provider,
    request: {
