...
export function validateExactHttpsUrl(value: string, allowedHosts: ReadonlySet<string>): URL {
  ...
  if (parsed.username || parsed.password)
    throw new Error('Provider media URL cannot contain credentials.');
...
export async function openBoundedGeneratedOutput(
  ...
  const contentType = (response.headers.get('content-type') ?? '')
    ...
    .trim()
    .toLowerCase();
  if (!acceptedOutputTypes.has(contentType))
    throw new Error('Provider output type is not supported.');
  ...
  if (contentLength > options.maximumBytes)
    throw new Error('Provider output exceeds the generated-media limit.');
  ...
  const limiter = new TransformStream<Uint8Array, Uint8Array>({
    transform(chunk, controller) {
      ...
      if (transferred > options.maximumBytes || transferred > contentLength) {
        controller.error(
          new Error('Provider output exceeded its declared or permitted size while streaming.')
        );
        return;
      }
    ...
    flush() {
      if (transferred !== contentLength)
        throw new Error('Provider output length did not match Content-Length.');
    },
  });
