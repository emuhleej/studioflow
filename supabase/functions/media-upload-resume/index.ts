  ...
  try {
    ...
    if (session.mode === 'single') {
      const uploadUrl = await getSignedUrl(
        client,
        new PutObjectCommand({ Bucket: bucket, Key: asset.storage_key }),
        { expiresIn: 15 * 60 }
      );
    ...
    const listed = await client.send(
      new ListPartsCommand({ Bucket: bucket, Key: asset.storage_key, UploadId: session.upload_id })
    );
    const completedParts = (listed.Parts ?? [])
      .filter(
        (part): part is typeof part & { ETag: string; PartNumber: number } =>
          Boolean(part.ETag) && Number.isInteger(part.PartNumber)
      )
      .map((part) => ({ ETag: part.ETag.replaceAll('"', ''), PartNumber: part.PartNumber }));
    await admin
      .from('upload_sessions')
      .update({ completed_parts: completedParts })
      .eq('asset_id', assetId)
      .eq('owner_id', user.id);
    return json(request, {
      mode: 'multipart',
