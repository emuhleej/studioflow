...
const inputSchema = z.object({
  ...
  parts: z
    .array(z.object({ ETag: z.string().min(1), PartNumber: z.number().int().positive() }))
    .max(10_000)
    .optional(),
});
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (session.state === 'completed')
      return json(request, { completed: true, alreadyCompleted: true });
    ...
    const uploaded = await client.send(
      new HeadObjectCommand({ Bucket: bucket, Key: asset.storage_key })
    );
