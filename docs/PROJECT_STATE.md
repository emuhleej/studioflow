...
Netlify serves the separately approved deployment `6aa5fc5761abe6000824f5fd` for commit `b9238422931b9be125f52dc898ccb86f5513a53a` at `https://studioflowhq.netlify.app`. It is published and locked, with Auto Publishing still disabled. Signed-out shell and `/health` checks pass without browser warnings or errors. The owner reports that production owner login and non-owner denial also pass; private-media expiry verification remains.
...
The adjustable one-image spending release is now published. PR #13 passed StudioFlow CI and merged as commit `b9238422931b9be125f52dc898ccb86f5513a53a`. Local verification passed formatting, TypeScript, ESLint, 124 unit/component tests across 28 files, six production-lock checks, the production build, and all 44 Playwright scenarios across desktop, both iPad orientations, and phone. `generation-start` version 16 is active with server-side model allowlisting and price recalculation. Exact Netlify deployment `6aa5fc5761abe6000824f5fd` completed through a one-time hook, was reviewed unpublished, and was manually published after action-time approval. The hook, its temporary local address, and `STUDIOFLOW_PRODUCTION_RELEASE_COMMIT` were removed. Netlify remains published and locked; the live signed-out shell and `/health` pass without browser errors. Generation remains disabled with zero active managed jobs, so the release made no Runway request and spent no credits.
...
- Netlify now publishes exact deployment `6aa5fc5761abe6000824f5fd` for approved commit `b9238422931b9be125f52dc898ccb86f5513a53a`. The deploy is locked and site-level Auto Publishing remains locked.
...
Quick Wins and AI-1 through AI-5 are complete, including the live encrypted version 2 backup/non-destructive restore rehearsal and the published adjustable one-image spending limit. The exact reviewed candidate for commit `b9238422931b9be125f52dc898ccb86f5513a53a` is published and locked as deployment `6aa5fc5761abe6000824f5fd`. Signed-out shell and health checks pass, and the owner reports that production owner login and non-owner denial pass. Generation is disabled with zero active managed jobs. The exact next checkpoint is private-media expiry verification. Do not publish a later commit or submit another provider request without separate approval.
